...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Minimize the cost of actuators
    for (int t = 0; t < N - 1; t++) {
    ...
    //   f(x[t]) - y[t] : current cross track error
    //   v[t] * sin(epsi[t]) * dt : change in error caused by vehicle movement
    // epsi[t+1] = psi[t] - psides[t] + v[t] * delta[t] / Lf * dt
    //   psi[t] - psides[t] : current orientation error
    //   v[t] * delta[t] / Lf * dt : change in error caused by vehicle movement
    for (int t = 1; t < N; t++) {
      ...
      if (t > 1) {
        // Account for latency, use the delta and a from previous step
      ...
      //desired psi - tangential angle of the polynomial f evaluated at x0
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Return the optimal x, y tranjectory
  for (int i = 0; i < N; i++) {
